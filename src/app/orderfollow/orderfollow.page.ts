...
@Component({
  selector: 'app-orderfollow',
  templateUrl: './orderfollow.page.html',
  styleUrls: ['./orderfollow.page.scss'],
})
export class OrderfollowPage implements OnInit, OnDestroy {
  ...
  constructor( 
    ...
    //this.orderID = this.navParams.get("örderID");
  }
  ...
  ionViewDidEnter() { 
    ...
    this.mapService.orderFollow(); // Firts data loading
    let self = this;
  ...
  getLocation(){
    let self = this;
    setInterval(function(){ self.mapService.getDeliveryLocation(); }, 10000);
  }
